function students(input) {
  ...
  for (const line of arrCopy) {
    ...
    if (checkData.length > 1) {
      ...
      const [usernameAndCredit, blank, blank2, email, blank3, courseName] = line.split(" ");
  ...
  Object.keys(courses)
    ...
    .map((key) => {
      courses[key].students = courses[key].students.sort((a, b) => b.credit - a.credit);
      return key;
    })
    .forEach((key) => {
      console.log(
        `${key}: ${courses[key].capacity} places left ${courses[key].students
