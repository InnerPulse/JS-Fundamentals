function foo(commands) {
    ...
    function haveCommonTechnique(a, b) {
    ...
    for (let i = 0; i < commands.length; i += 1) {
        ...
        if (skill !== undefined) {
            ...
            if (gladiatorsPool[name][skill] < Number(skillValue))
                gladiatorsPool[name][skill] = Number(skillValue);
        } else {
            const [glad1, glad2] = name.split(' vs ');
            if (doesExist(glad1) && doesExist(glad2)) {
                if (haveCommonTechnique(glad1, glad2)) {
                    sumSkill(Object.entries(gladiatorsPool[glad1])) >
                    sumSkill(Object.entries(gladiatorsPool[glad2]))
