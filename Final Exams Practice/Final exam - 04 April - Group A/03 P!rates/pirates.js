function pirates(input) {
  ...
  const eventsFunctions = {
    ...
    Plunder: (data) => {
      ...
      logs.push(`${town} plundered! ${stolenGold} gold stolen, ${killed} citizens killed.`);
    ...
    Prosper: (data) => {
      ...
      Number(goldIncreased) < 0
        ...
        : logs.push(`${goldIncreased} gold added to the city treasury. ${town} now has ${targets[town].gold} gold.`);
    },
  };
  ...
  if (Object.keys(targets) < 1) {
    logs.push(`Ahoy, Captain! All targets have been plundered and destroyed!`);
  } else {
    logs.push(`Ahoy, Captain! There are ${Object.keys(targets).length} wealthy settlements to go to:`);
    const sorted = Object.entries(targets)
      .sort(compare)
      .forEach((town) =>
        logs.push(`${town[0]} -> Population: ${town[1].population} citizens, Gold: ${town[1].gold} kg`)
      );
  }
